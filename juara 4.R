...
MODISGRID<-'+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181 +units=m +no_defs'
...
ex42<-extent(c( 366971.9, 403203.4, 8811367 ,8845732) )
ex42sp<-as(ex42, "SpatialPolygons")
...
dc2<-spTransform(dc,CRS(UTM21S))
dc3<-spTransform(dc2,CRS(MODISGRID))
...
#49:143
#3333+167 rows for ndvi; change in seasonality: 25
#1992 for evi 2 seasonality; 5843+3333 for trend
...
### classification map
s1269=readOGR("C:/TC_MT_2012_22869/TC_MT_2012_22869__pol.shp", layer="TC_MT_2012_22869__pol") #will load the shapefile to your dataset.
s1268=readOGR("C:/TC_MT_2012_22868/TC_MT_2012_22868__pol.shp", layer="TC_MT_2012_22868__pol") #will load the shapefile to your dataset.
s0869=readOGR("C:\\terraclass_2008_22869__pol.shp", layer="terraclass_2008_22869__pol") #will load the shapefile to your dataset.
s0868=readOGR("C:\\terraclass_2008_22868__pol.shp", layer="terraclass_2008_22868__pol") #will load the shapefile to your dataset.

#s0868<-spTransform(s0868,CRS(proj4string(s1268)))

#lists0868<-s0868@polygons[which(s0868@data$tcclasse=='FLORESTA')]
sp0868cl<-SpatialPolygons(lists0868)
#combine 68 69 for the year 2008
lists0868<-s0868@polygons[which(s0868@data$tcclasse=='FLORESTA')]
sp0868cl<-SpatialPolygons(lists0868) 

lists0869<-s0868@polygons[which(s0869@data$tcclasse=='FLORESTA')]
sp0869cl<-SpatialPolygons(lists0869) 
sp0869cla<- spChFIDs(sp0869cl , as.character(paste('C',c(1:length(sp0869cl )),sep='')))

bind086869<-spRbind(sp0869cla,sp0868cl)
extent (bind086869)
# combine 68 69 for the year 2012
lists1268<-s1268@polygons[which(s1268@data$tc_2012=='FLORESTA')]
sp1268cl<-SpatialPolygons(lists1268) 

lists1269<-s1268@polygons[which(s1269@data$tc_2012=='FLORESTA')]
sp1269cl<-SpatialPolygons(lists1269) 
sp1269cla<- spChFIDs(sp1269cl , as.character(paste('B',c(1:length(sp1269cl )),sep='')))

bind126869<-spRbind(sp1269cla,sp1268cl)
##

#bind0812<-spRbind(sp1268cla,sp0868cl)

proj4string(bind08126869)<-CRS(proj4string(s1268))
bind08126869<-spTransform(bind0812,CRS("+proj=utm +zone=21 +south  +ellps=WGS84 "))
plot(bind08126869,add=TRUE)
extent(bind08126869)
#class       : Extent 
#xmin        : 314920.8 
#xmax        : 510541.6 
...
??spRbind
C:\Users\m_lu0002\Desktop\climate\minnesota
#print(length( shape@polygons[prodes1$class_name=='DESFLORESTAMENTO']))
#names(shape)
plot(shape["tc_2012"]) # forst
#modis sinusoidal
#+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181 +units=m +no_defs
#utm zone 21 south
shape.utm<-spTransform(shape,CRS(UTM21S))
shape.modis<-spTransform(shape.utm,CRS(MODISGRID))
plot(shape.modis)
...
proj4string(xyms1)<-MODISGRID
krigc1<-spTransform(xyms1,  CRS(UTM21S))
...
{  
  ...
  { 
    ...
    na.new <- function(x) ts(na.exclude(x), frequency = 12) # zoo is irregular
    
    stlmon<-stl(monmean2, na.action = na.new, s.window = "per")
    datamon <- ts(rowSums(stlmon$time.series),start=c(2000,02),end=c(2013,12),frequency=12) 
   
   #tsp(stlmon$time.series)
    
    fitmon <- bfast(datamon,h=0.15, season="harmonic", max.iter=3) 
# plot(fitmon)
    #fitmon$output[[1]]$Vt.bp
...
pv1t2<-array(,c(150,150))  
pv1s2<-array(,c(150,150)) # pvalue of evi 150 by 150 use monthly mean



#evi8<-as.numeric(evi7[,,])
#fevi8<-filter.st.median(evi8)
#evi8<-apply(evi7, 1:3,as.numeric)
...
a10t<-array(,c(10,10,167))
a10s<-array(,c(10,10,167))
a10t2<-array(,c(10,10,167))
a10s2<-array(,c(10,10,167))
date.trend2<-data.frame()
date.seasonal2<-data.frame()
i4=1
 load("a1.saved")
...
array1<-fevi8[8:10,8:10,]
ts1<-fevi8[9,9,]
ts1<-ts(ts1,frequency=46)
bfast(ts1,season="harmonic",max.iter=1)

coefarray<-array(,c(3,3))
coefarray2<-array(,c(3,3))
for (i in 1:3)
{
  for ( j in 1:3)
  {
    ts1<-array1[i,j,]
    ts1<-zoo( ts1,a1)
    monmean2 <- aggregate(ts1, as.Date(as.yearmon(a1)), mean) #should aggregate in SciDB
    frequency(monmean2)<-12
    na.new <- function(x) ts(na.exclude(x), frequency = 12)
    stlmon<-stl(monmean2, na.action = na.new, s.window = "per")
    datamon <- ts(rowSums(stlmon$time.series)) 
    tsp(datamon) <- tsp(stlmon$time.series)
    t=1:length(datamon)
    coefarray[i,j]<-coef(lm(datamon~t))[2]
  }
} 
image(coefarray)
for (i in 1:3)
...
image(coefarray2)
str(try1)
t1sp<-as.data.frame(as.table(coefarray))
levels(t1sp$Var1)<-seq(1,3)
levels(t1sp$Var2)<-seq(1,3)
...
#nb1<-knn2nb(knearneigh(c1,k=1))
cn<-cell2nb(3,3, type ="queen",torus =FALSE)
# str(cn)
listw1<-nb2listw(cn)
#str(listw1)
  array2<-array(t1sp$Freq,c(9,1))
#try1<-spautolm(array2~1,listw=listw1)
(try1)
coefarray2<-array(fitted(try1),c(3,3))

#jpeg(paste('map seasonal parameter',sepa[i],'.jpg'), height=5, width=5, res=400,unit="in")
#spplot(t1sp)

  sctest(efp(datamon~t))
...
str(array1)
length(which(a10t!=0))
which(a10t!=0,arr.ind=TRUE)
length(which(a10t2!=0))
which(a10t2!=0,arr.ind=TRUE)
...
save( a150p1s,file='a150p1s.RData')
save( a150p1t,file='a150p1t.RData')
...
######for how many changed time series (and for filtered p value) 
i4=1
...
{  
  ...
  fitmon <- bfast(datamon,h=0.15, season="harmonic", max.iter=3,alpha=0.1) 
...
##########################################################
save(fopt1,file='fopt-pf2.RData')
save(fops1,file='fops1.RData')
length(which(!is.na(fopt1))) #657
...
load( 'pv1s2.RData')
level<-c(0.05,0.1)
fpv1t2<-pfilter2(pv1t2,level)
fpv1s2<-pfilter2(pv1s2,level)

filtered.pv1t<-fpv1t2[[1]]
filtered.pv1s<-fpv1s2[[1]]

keeptrack.pv1t<-fpv1t2[[2]]
keeptrack.pv1s<-fpv1s2[[2]]
...
length(fil.pls1) #seasonlity 1212 trend 1456
which(pv1s2[which(keeptrack.pv1s==2.0)]<=0.05)
...
################################################################
pfilter1 <-function(a,val=0.05) # a: p value matrix (of each x y)
{
   b<-a
  row<-length(a[,1])-1
  col<-length(a[,2])-1
...
{ 
    ...
    proj4string(a1)<-MODISGRID
    #a1<-spTransform(a1,  CRS(UTM21S))
    #a2<-cbind(a2,a1)
...
{  
 ...
 i<-as.integer(index1[(1+nr2[i2]):( nr2[i2+1] ),1]) 
  ...
  a1<-c()
    ...
    proj4string(xyms1)<-MODISGRID
    a1<-spTransform(xyms1,  CRS(UTM21S))
...
#spplot(deter1)
detert1<-spTransform(deter1,CRS(UTM21S))
...
#  load("mevi2t.Rdata")
changearray=fopt1
length(bfastchangepoint(a150p05t)) # 6251
plot(bfastchangepoint(fopt1),col='skyblue2',add=TRUE)

bfastchangepoint(a150p05s)[bfastchangepoint(fopt1),]
...
proj4string(spmodist51)<-MODISGRID

modis.mt52<-spTransform(spmodist51,CRS(UTM21S))
return(modis.mt52)
}
...
data.first<-spTransform(data.first,CRS(UTM21S))
...
for (i in 4:length(deterg)) {
     ...
     data.temp<-spTransform(data.temp,CRS(UTM21S))
...
proj4string(spatialPolygons)<-CRS(UTM21S)
setwd( "C:/Users/m_lu0002/Desktop/Climate/minnesota/juara/deter") 
...
detert1<-spTransform(com,CRS(UTM21S))
################################################################
plot(com)
...
#save(s6,file='s6.Rdata') # a raster
plotRGB(crop(s6,epu4))
plot(krigc1)
s6
...
#color=c('skyblue','red','gold','yellow','blue','antiquewhite2','darkolivegreen1','darkgreen','purple','pink','chartreuse1','deepskyblue4')
setwd("C:/Users/m_lu0002/Desktop/Climate/PRODES data")
setwd("C:/Users/m_lu0002/Desktop/Climate/DEGRAD")
# PRODES
prodesfile<-list.files(pattern = "\\.shp$")
prodesf<-prodesfile 
#prodesfile<-list.files(pattern = "\\.shp$") 
lprodes<-substr(prodesf, start=1, stop=nchar(prodesf)-4) #layer
for (i in 1:7)
{

prodes3=readOGR(prodesf[i], layer=lprodes[i]) #will load the shapefile to your dataset.
prodes1=c(prodes1,prodes3)
prodes1=readOGR(prodesf[i],layer=lprodes[i] )
plot(prodes1)
#length(prodes1@polygons) 

#Spa1<-c(SpatialPolygons( prodes1@polygons[prodes1$class_name=='DESFLORESTAMENTO']))
print(length( prodes1@polygons[prodes1$class_name=='DESFLORESTAMENTO']))
if (length( prodes1@polygons[prodes1$class_name=='DESFLORESTAMENTO'])!=0)
  #   plot(SpatialPolygons(prodes1@polygons[prodes1$class_name=='DESFLORESTAMENTO']),col=color2[i])
  plot(SpatialPolygons(prodes1@polygons[prodes1$class_name=='DESFLORESTAMENTO']),add=TRUE,col=color2[i])
}
str(prodes1)
#jpeg('around520evi2bfast061007vspoly2007all.jpg', height=12, width=20, res=800,unit="in")


x<-c(59138:59180) # 4240
y<-c(48712:48752)

x<-c(58930:59079) #150 150
y<-c(48210:48369)

proj4string(prodes67808)<-CRS('+init=epsg:4618')
prodes67808tr<-spTransform( prodes67808,CRS("+proj=utm +zone=21 +south  +ellps=WGS84 "))

plot(prodes67808tr)
plot(ex42,add=TRUE)
crop(prodes67808tr,ex42sp)

ex42<-extent(c( 366971.9, 403203.4, 8811367 ,8845732) )
ex42sp<-as(ex42, "SpatialPolygons")
proj4string(ex42sp)<-CRS("+proj=utm +zone=21 +south  +ellps=WGS84 ")

crprod67808<-gIntersection(prodes67808tr, ex42sp, byid=TRUE)
plot(crprod67808)

rasterize250mprodes08<- polygontopoint(x,y,prodes67808) # 579 

crprod67808points<-crop(rasterize250mprodes08,ex42)
plot(crprod67808)
plot(crprod67808points,add=TRUE,col='yellow')
plot(ex42,add=TRUE)
plot(modis.mt52,add=TRUE)
save(crprod67808,file='crprod67808.Rdata')
save(crprod67808points,file='crprod67808points.Rdata')

#bmonitor0709<-which(
 #
monitor1t[!is.na(monitor1t)][which( which(!is.na(monitor1t),arr.ind=TRUE)[,3]<414)] 
#2007 02: 322  2008: 368  2009: 414
#compare with bfastmonitoring
timeind<-c(368,414,460)
pvsdm(2)
 i=2
pvsdm<-function(i){
  timeextent<-length(which( which(!is.na(monitor01t08) ,arr.ind=TRUE)[,3]<timeind[i]))

  timeextent8<-length(which( which(!is.na(monitor01t08) & monitor01t08==8,arr.ind=TRUE)[,3]<timeind[i]))
timeextent7<-length(which( which(!is.na(monitor01t08) & monitor01t08==7,arr.ind=TRUE)[,3]<timeind[i]))
timeextent6<-length(which( which(!is.na(monitor01t08) & monitor01t08==6,arr.ind=TRUE)[,3]<timeind[i]))
timeextent5<-length(which( which(!is.na(monitor01t08) & monitor01t08==5,arr.ind=TRUE)[,3]<timeind[i]))
timeextent4<-length(which( which(!is.na(monitor01t08) & monitor01t08==4,arr.ind=TRUE)[,3]<timeind[i]))
timeextent3<-length(which( which(!is.na(monitor01t08) & monitor01t08==3,arr.ind=TRUE)[,3]<timeind[i]))
timeextent2<-length(which( which(!is.na(monitor01t08) & monitor01t08==2,arr.ind=TRUE)[,3]<timeind[i]))
timeextent1<-length(which( which(!is.na(monitor01t08) & monitor01t08==1,arr.ind=TRUE)[,3]<timeind[i]))

timeextent<-length(which( which(!is.na(monitor1t08) ,arr.ind=TRUE)[,3]<timeind[i])) #414: 1448
timeextent2<-length(which( which(!is.na(monitor1t08) & monitor1t08==8,arr.ind=TRUE)[,3]<timeind[i]))
#load('monitor1tt.Rdata')
monitor1t08<-monitor01t08

timeextentlist<-c(timeextent1,timeextent2,timeextent3,timeextent4,timeextent5,timeextent6,timeextent7,timeextent8)

bmoitpointsclassify<-function(i)
{
  x<- which(!is.na(monitor1t08) & monitor1t08==i,arr.ind=TRUE)[,1][1:timeextentlist[i]] #2907 #1313 # 380
y<-which(!is.na(monitor1t08)& monitor1t08==i,arr.ind=TRUE)[,2][1:timeextentlist[i]]
xct1<-x+58929 # for the second 150 by 150 array
yct1<-y+48209
xychangeinmodist<-as.data.frame(cbind(xct1,yct1))
changeinmot0.5.1<-getxyMatrix(xychangeinmodist,231.6564)
spmodist51<-SpatialPoints(coordinates(changeinmot0.5.1))
proj4string(spmodist51)<-MODISGRID
modis.mt60<-spTransform(spmodist51,CRS("+proj=utm +zone=21 +south  +ellps=WGS84 "))
return(modis.mt60)
}
i=1
paste("modis.mt",i,sep="")<-bmoitpointsclassify(i)
 
xall<- which(!is.na(monitor1t08),arr.ind=TRUE)[,1][1:timeextent] #2907 #1313 # 380

yall<-which(!is.na(monitor1t08 ),arr.ind=TRUE)[,2][1:timeextent]


 
jpeg( paste('prodes08vs.bfastmonirtoringstag2475all',i,'.jpg'),height=1200,width=1200)

plot(crprod67808) # prodes polygons
plot(crprod67808points,add=TRUE,col='yellow') # prodes points
#points(rasterize250mprodes08,col='orange') #prodes
points(modis.mt54,col='pink')
points(modis.mt56,col='skyblue4')

points(modis.mt53,col='green')#bfastmonitoring
points(rasterize250mprodes08[modis.mt56,],col='skyblue4',pch=1)
points(rasterize250mprodes08[modis.mt57,],col='green',pch=17)
points(rasterize250mprodes08[modis.mt58,],col='red',pch=18)
points(rasterize250mprodes08[modis.mt59,],col='black',pch=15)
points(rasterize250mprodes08[modis.mt60,],col='blue',pch=15)


points(rasterize250mprodes08[modis.mt56,],col='purple',pch=16)
legend('bottomright',legend=c('deterall','bfast all','overlap bfast all','overlap bfast tag 05',
                           
                              'overlap bfast tag 07','overlap bfast tag 04',
                              'overlap bfast tag 02'),
       col=c('yellow','skyblue4','purple','green','red','black','blue'),pch=c(3,1,16,17,18,15,15))
dev.off()
print(length(rasterize250mprodes07[modis.mt52,])) #07: 26 # 0708 76 #070809 136
#bfast start at 2008 : deter 07 104
}
points(rasterize250mprodes08[modis.mt52,],col='purple') #192 #79 # 31 49
rasterize250mprodes07<- polygontopoint(x,y,prodes67807) # 579 
rasterize250mprodes08
save(rasterize250mprodes08,file='rasterize250mprodes08')
save(rasterize250mprodes07,file='rasterize250mprodes07')
 
############
 
plot(crprod67808)
points(rasterize250mprodes08)
#crprod67808<-crop(prodes67808tr,ex42) # raster wont work for polygons ( eighter rasterize or use gintersect in rgeo)

ex42<-extent(c( 366971.9, 403203.4, 8811367 ,8845732) )
cr250mp07<-crop(rasterize250mprodes07,ex42sp)


points(cr250mp08) # 150 by 150 change point in prodes
save(cr250mp08,file='cr250mp08')
# 30m polygon-> 250 m points: will miss information
# but to compare with bfast change
# are there better ways?
proj4string(prodes67808)<-CRS('+init=epsg:4618')
polygontopoint<-function(x,y,sppolygon) {
  ...
  proj4string(xyc)<-MODISGRID
  xycs<-spTransform(xyc,CRS("+proj=utm +zone=21 +south  +ellps=WGS84 "))
  plot(sppolygon)
  sppolygon<-spTransform( sppolygon,CRS("+proj=utm +zone=21 +south  +ellps=WGS84 "))
   points<-xycs[sppolygon,]
  plot(xycs,add=TRUE) 
  return( points)
}
plot(xycs,add=TRUE)
...
extent(xycs)
plot(sppolygon)
color2=rep(c('skyblue','red','gold','yellow','blue','antiquewhite2','darkolivegreen1','darkgreen','purple','pink','chartreuse1','deepskyblue4'),10)
prodesf22867<-prodesf[grep('_22867',prodesf )]
prodesf22868<-prodesf[grep('_22868',prodesf )]
lprodes22867<-substr(prodesf22867, start=1, stop=nchar(prodesf)-4) #layer
lprodes22868<-substr(prodesf22868, start=1, stop=nchar(prodesf)-4)
setwd("C:/Users/m_lu0002/Desktop/Climate/PRODES data")
for (i in 1 :length(prodesf))
{
 i=1 #i=1: 2007
 prodes67=readOGR(prodesf22867[i],layer=lprodes22867[i] )
 prodes68=readOGR(prodesf22868[i] ,layer=lprodes22868[i] )
 proj4string(prodes67)<-CRS()
 proj4string(prodes68)<-CRS()
 prodes67A <- spChFIDs(prodes67, as.character(paste('A',c(1:length(prodes67)),sep='')))
 prodes678<- spRbind(prodes67A,prodes68) #228 67 and 68
 
 print( levels(prodes678$SPRCLASSE))
}
listp67807<- prodes678@polygons[prodes678$class_name=='DESFLORESTAMENTO']
length(listp67807)
str(prodes67807)
prodes67807<-SpatialPolygons(listp67807)
prodes67808<-SpatialPolygons(listp67807)
 plot(prodes67808)

save(prodes67807,file='prodes67807.Rdata')
save(prodes67808,file='prodes67808.Rdata')
load(file='prodes67807')
load(file='prodes67808')
levels(prodes678$class_name)
#for PRODES
# "desmatamento_total" "dsf_nv_02"          "floresta"          
# "hidrografia"        "nao_floresta"       "nuvem"             
# "dsf_nv_03" 

# prodes678@polygons[[i]]@area
levels(prodes678$SPRCLASSE)
#listp678<- prodes678@polygons[prodes678$SPRCLASSE=='desmatamento_total']
#listp67807<- prodes678@polygons[prodes678$class_name=='DESFLORESTAMENTO']
#prodes678@polygons[prodes678$SPRCLASSE=='desmatamento_total'][[1]]   
#prodes1@polygons[prodes1$class_name=='FLORESTA'] 
#  
 
shapefile(prodesf[20])
for (i in 1:length(prodesf))
{
  # j=i+0
  #deterall[j]
...
#prodes1[[1]][prodes1[[2]],]
 # proj4string(prodes1)<-CRS('+init=epsg:4618')
  # detert1<-spTransform(deter1,CRS(UTM21S))
 # prodes1<-spTransform(prodes1,CRS("+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))
 

#Spa1<-c(SpatialPolygons( prodes1@polygons[prodes1$class_name=='FLORESTA']))
...
spatialPolygons <- SpatialPolygons(polygons)
setwd( "C:/Users/m_lu0002/Desktop/Climate/minnesota/juara/deter") 
save(spatialPolygons,file='SpatialPolygons.R')
load('Spatial1.R')
...
plot(modis.mt52[spatialPolygons,]
     ...
     ep4<-as(ex4,'SpatialPolygons')
     proj4string(ep4)<-'+proj=utm +zone=21 +south +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0'
     ex150usgs<-spTransform(ex42sp,CRS("+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))
     epu4<-spTransform(ep4,CRS("+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))
     usgspro<-"+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "
     ...
     load('xy2')
     setwd("C:/Users/m_lu0002/Desktop/Climate/minnesota")
     #save(evi8,file='evi8.Rdata')
     #save(fevi8,file='fevi8.Rdata')
     load('fevi8.Rdata')
     #save(a1, file="a1.saved") 
     #save(ndviallr, file ='ndviallr.saved')
length(which(!is.na(or4240p1t)))
ex150usgs<-spTransform(ex42sp,CRS("+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))
epu4<-spTransform(ep4,CRS("+proj=utm +zone=21 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))