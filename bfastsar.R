bfastsar <- function(starr,  h=0.15 , season =c("dummy","harmonic","none"), max.iter = NULL, breaks = NULL, hpc = "none", level = 0.05, type= "OLS-MOSUM")
{
  le=636
  tl=1:le
  w=1/46
  ...
  X = matrix(0, 636 * 9, 9)
  X[seq(1,by=9,length.out=636),1] = tl
  X[seq(2,by=9,length.out=636),2] = tl
  X[seq(3,by=9,length.out=636),3] = tl
  X[seq(4,by=9,length.out=636),4] = tl
  X[seq(5,by=9,length.out=636),5] = tl
  X[seq(6,by=9,length.out=636),6] = tl
  X[seq(7,by=9,length.out=636),7] = tl
  X[seq(8,by=9,length.out=636),8] = tl
  X[seq(9,by=9,length.out=636),9] = tl
  colnames(X) = paste0("v", 1:9)
  
 
  X2 = matrix(0, 636 * 9, 54)
  
  for( i in 1:9)
  {
    
 
    X2[seq(i,by=9,length.out=636),1+(i-1)*6] =co
    X2[seq(i,by=9,length.out=636),2+(i-1)*6] =co2
    X2[seq(i,by=9,length.out=636),3+(i-1)*6] =co3
    X2[seq(i,by=9,length.out=636),4+(i-1)*6] =si
    X2[seq(i,by=9,length.out=636),5+(i-1)*6] =si2
    X2[seq(i,by=9,length.out=636),6+(i-1)*6] =si3
  }
  
  colnames(X2) = paste0("v2", 1:54)
  ...
  while ( (!identical(CheckTimeTt,Vt.bp) | !identical(CheckTimeSt,Wt.bp)) & i < max.iter)
  {
   ...
   if (p.Vt$p.value <= level[1]) 
    ...
    } else
    {
      ...
      fevi3b312t<-apply(starr,c(1,2),function(x) (ts(x,start=c(2000,1),frequency=46)-Tt))
      f2<-aperm(fevi3b312t,c(2,3,1))  
      aa2<-as.vector(f2) 
      try2<-spautolm(aa2~. , data.frame(aa2,X2),family="SAR",method= "Matrix", listw=listcn636)   
      rn<-lapply(1:9,function(i) {residuals(try2)[seq(i,636*9-(9-i),9)]})
      Wt<-ts(f2[2,2,],start=c(2000,1),frequency=46) # reconstruct the time series
     # Wt <- Yt-Tt
      p.Wt <- sctest(efp(smod, h=h, type=type,spatial1=rn[[1]]))      # preliminary test 
